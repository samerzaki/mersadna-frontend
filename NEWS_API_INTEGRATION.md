...
The news section has been successfully integrated with the Nezzel backend API. News articles are now fetched from the `/api/news/scraped` endpoint instead of using mock data.
...
curl -H "Accept: application/json" \
     ...
     "http://nezzel.test/api/news/scraped?page=1"

# Get single news
curl -H "Accept: application/json" \
     -H "Accept-Language: ar" \
     "http://nezzel.test/api/news/scraped/1"

# Search
curl -H "Accept: application/json" \
     -H "Accept-Language: ar" \
     "http://nezzel.test/api/news/scraped?search=ذهب"
```
